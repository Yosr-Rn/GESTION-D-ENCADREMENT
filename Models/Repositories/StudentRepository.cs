...
namespace GestionEtudiant.Models.Repositories
{
    public class StudentRepository:IStudentRepository
    {
        ...
        {
            return context.Students.OrderBy(x => x.StudentName).Include
                (x => x.School).Include(x => x.Teacher).ToList();
        }
        public Student GetById(int id)
        {
            return context.Students.Where(x => x.StudentId == id).Include(x => x.School).Include(x=>x.Teacher).SingleOrDefault();
        }
        ...
        public IList<Student> GetStudentsByTeacherID(int? teacherid)
        {
            return context.Students.Where(s =>
           s.TeacherID.Equals(teacherid))
           .OrderBy(s => s.StudentName)
           .Include(std => std.Teacher).ToList();
        }
    }
}